...
// Accès sécurisé à la clé API pour Vercel (VITE_ prefixé pour le client)
const getApiKey = () => {
  try {
    // @ts-ignore
...
// --- COMPOSANTS UI (DÉFINIS ICI POUR ÉVITER ReferenceError) ---
...
const CornerTriangle = ({ customLogo }) => (
  <div className="absolute top-0 left-0 w-[170px] h-[170px] z-50 pointer-events-none print:w-[150px] print:h-[150px]">
    ...
    {customLogo && (
      <div className="absolute top-[12px] left-[12px] w-[100px] h-[100px] flex items-center justify-center text-left">
...
export default function App() {
  ...
  const [cvData, setCvData] = useState(() => {
    try {
      const saved = localStorage.getItem('smile_cv_stability_v12_final');
      if (saved) return JSON.parse(saved);
    } catch(e) { console.error(e); }
    return DEFAULT_CV_DATA;
  });

  useEffect(() => { localStorage.setItem('smile_cv_stability_v12_final', JSON.stringify(cvData)); }, [cvData]);
  ...
  const handlePDFImport = async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    if (!apiKey) { alert("Clé API VITE_GOOGLE_API_KEY non configurée."); return; }
    setIsImporting(true);
    try {
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row h-screen overflow-hidden font-sans">
      ...
      <div className="w-full md:w-[500px] bg-white border-r border-slate-200 flex flex-col h-full z-10 shadow-xl print:hidden text-left">
        <div className="bg-slate-50 border-b border-slate-200 p-2 flex justify-between items-center px-4">
           ...
           <ButtonUI variant={cvData.isAnonymous ? "danger" : "secondary"} className="px-2 py-1 h-7" onClick={() => setCvData(p => ({...p, isAnonymous: !p.isAnonymous}))}><Lock size={12}/> {cvData.isAnonymous ? "Anonyme" : "Visible"}</ButtonUI>
        </div>
        ...
        <div className="p-6 border-b border-slate-100 bg-white sticky top-0 z-20">
          ...
          <div className="flex gap-2">
            <ButtonUI variant="secondary" onClick={() => setStep(s => Math.max(1, s - 1))} disabled={step === 1} className="flex-1 text-left"><ArrowLeft size={16} /></ButtonUI>
            {step < 4 ? <ButtonUI onClick={() => setStep(s => Math.min(4, s + 1))} className="flex-[2] text-left">Suivant <ArrowRight size={16} /></ButtonUI> : <ButtonUI onClick={handlePrintPDF} className="flex-[2] bg-slate-900 text-white shadow-lg font-bold text-left"><Printer size={16} /> Générer PDF</ButtonUI>}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-8 custom-scrollbar">
             ...
             <div className="space-y-6 animate-in slide-in-from-right duration-300 text-left">
                <div className="grid grid-cols-2 gap-4 text-left">
                  <div className="flex flex-col gap-2 text-left">
                    <span className="text-[10px] font-bold text-slate-400 uppercase text-left">Logo Entreprise</span>
                    <DropZoneUI onFile={handleSmileLogo} label="Logo" />
                  </div>
                  <div className="flex flex-col gap-2 text-left text-left">
                    <span className="text-[10px] font-bold text-slate-400 uppercase text-left">Ma Photo</span>
           ...
           {step === 2 && (
            <div className="space-y-6 animate-in slide-in-from-right duration-300 text-left text-left">
               <h2 className="font-bold text-lg text-slate-700 flex items-center gap-2 text-left uppercase tracking-tighter text-left"><Hexagon size={20}/> Soft Skills</h2>
           ...
           {step === 3 && (
             <div className="space-y-8 animate-in slide-in-from-right duration-300 text-left text-left">
               <h2 className="font-bold text-lg text-slate-700 flex items-center gap-2 text-left uppercase tracking-tighter text-left text-left"><GraduationCap size={20}/> Formation & Compétences</h2>
               <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm text-left text-left">
                 <h3 className="text-[10px] font-black uppercase text-slate-400 mb-4 text-left text-left">Secteur & Certifs</h3>
                 <div className="flex gap-2 mb-4 text-left text-left">
                 ...
                 <div className="flex flex-wrap gap-1 mb-4 text-left">{(cvData.connaissances_sectorielles || []).map((s, i) => (<span key={i} className="bg-white text-[9px] font-bold px-2 py-0.5 rounded border flex items-center gap-1 uppercase">{s} <X size={10} className="cursor-pointer" onClick={() => removeSecteur(i)}/></span>))}</div>
               ...
               <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 shadow-sm text-left">
                 ...
                 {(cvData.education || []).map((edu, i) => (<div key={i} className="bg-white p-3 rounded-lg border mb-3 relative group shadow-sm text-left"><button onClick={() => removeEducation(i)} className="absolute top-2 right-2 text-slate-300 hover:text-red-500 text-left"><Trash2 size={12}/></button><InputUI label="Diplôme" value={edu.degree} onChange={v => updateEducation(i, 'degree', v)} /><div className="grid grid-cols-2 gap-2 text-left"><InputUI label="Année" value={edu.year} onChange={v => updateEducation(i, 'year', v)} /><InputUI label="Lieu" value={edu.location} onChange={v => updateEducation(i, 'location', v)} /></div></div>))}
      ...
      {/* --- PREVIEW AREA --- */}
      <div className="flex-1 bg-slate-800 overflow-hidden relative flex flex-col items-center border-l border-slate-700">
        ...
        <div className="flex-1 overflow-auto w-full p-8 flex justify-center custom-scrollbar text-left text-left">
          <div className="print-container flex flex-col origin-top transition-transform duration-300 gap-10" style={{ transform: `scale(${zoom})`, marginBottom: `${zoom * 100}px` }}>
            ...
            <A4Page>
              ...
              <div className="flex-1 flex flex-col justify-start pt-0 pb-12 overflow-hidden text-center text-center text-center">
            ...
            <A4Page>
              ...
              <div className="grid grid-cols-12 gap-10 mt-20 h-full px-12 flex-1 pb-32 overflow-hidden print:overflow-visible text-left text-left text-left">
                  <div className="col-span-5 border-r border-slate-100 pr-8 text-left text-left text-left text-left">
                    <h3 className="text-lg font-bold text-[#2E86C1] uppercase tracking-wide font-montserrat mb-8 flex items-center gap-2 text-left text-left text-left text-left"><Cpu size={20}/> Mes Compétences</h3>
                    <div className="space-y-8 text-left text-left text-left">{Object.entries(cvData.skills_categories).map(([cat, skills]) => (<div key={cat}><h4 className="text-[10px] font-bold text-[#999999] uppercase tracking-widest border-b border-slate-100 pb-2 mb-3 text-left text-left text-left">{cat}</h4><div className="space-y-3 text-left">{(skills || []).map((skill, i) => (<div key={i} className="flex items-center justify-between text-left text-left text-left text-left"><span className="text-xs font-bold text-[#333333] uppercase text-left text-left text-left">{String(skill.name)}</span><HexagonRating score={skill.rating} /></div>))}</div></div>))}</div>
                  </div>
                  <div className="col-span-7 flex flex-col gap-10 text-left text-left text-left">
                    {cvData.showSecteur && (cvData.connaissances_sectorielles || []).length > 0 && (<section className="text-left text-left text-left text-left"><h3 className="text-lg font-bold text-[#2E86C1] uppercase tracking-wide font-montserrat mb-4 flex items-center gap-2 text-left text-left text-left"><Factory size={20}/> Secteurs</h3><div className="flex flex-wrap gap-2 text-left">{(cvData.connaissances_sectorielles || []).map((s, i) => (<span key={i} className="border-2 border-[#2E86C1] text-[#2E86C1] text-[10px] font-black px-3 py-1 rounded uppercase tracking-wider text-left">{String(s)}</span>))}</div></section>)}
                    {cvData.showCertif && (cvData.certifications || []).length > 0 && (<section className="text-left text-left text-left text-left text-left"><h3 className="text-lg font-bold text-[#2E86C1] uppercase tracking-wide font-montserrat mb-4 flex items-center gap-2 text-left text-left text-left text-left"><Award size={20}/> Certifications</h3><div className="grid grid-cols-2 gap-4 text-left text-left text-left">{cvData.certifications.map((c, i) => (<div key={i} className="flex items-center gap-3 bg-slate-50 p-2 rounded text-left text-left text-left">{c.logo && <img src={c.logo} className="w-8 h-8 object-contain" alt={String(c.name)} />}<span className="text-[10px] font-bold text-slate-700 uppercase leading-tight text-left text-left text-left">{String(c.name)}</span></div>))}</div></section>)}
                    <section className="text-left text-left text-left text-left text-left text-left"><h3 className="text-lg font-bold text-[#2E86C1] uppercase tracking-wide font-montserrat mb-6 flex items-center gap-2 text-left text-left text-left text-left text-left"><GraduationCap size={20}/> Ma Formation</h3><div className="space-y-4 text-left text-left text-left text-left text-left">{(cvData.education || []).map((edu, i) => (<div key={i} className="border-l-2 border-slate-100 pl-4 text-left text-left text-left text-left text-left text-left"><span className="text-[10px] font-bold text-[#999999] block mb-1 text-left text-left text-left text-left text-left">{String(edu.year)}</span><h4 className="text-xs font-bold text-[#333333] uppercase leading-tight text-left text-left text-left text-left text-left">{String(edu.degree)}</h4><span className="text-[9px] text-[#2E86C1] font-medium uppercase text-left text-left text-left text-left text-left">{String(edu.location)}</span></div>))}</div></section>
                  </div>
              </div>
              <Footer />
            </A4Page>
            ...
            {experiencePages.map((chunk, pageIndex) => (
              <A4Page key={pageIndex}>
                <CornerTriangle customLogo={cvData.smileLogo} />
                <HeaderSmall isAnonymous={cvData.isAnonymous} profile={cvData.profile} role={cvData.profile.current_role} />
                <div className="flex justify-between items-end border-b border-slate-200 pb-2 mb-8 mt-16 px-12 flex-shrink-0 text-left text-left text-left text-left text-left text-left text-left"><h3 className="text-xl font-bold text-[#2E86C1] uppercase tracking-wide font-montserrat">{pageIndex === 0 ? "Mes dernières expériences" : "Expériences (Suite)"}</h3><span className="text-[10px] font-bold text-[#666666] uppercase">Références</span></div>
                <div className="flex-1 px-12 pb-32 overflow-hidden print:overflow-visible text-left text-left text-left text-left text-left text-left text-left">{chunk.map((exp) => (<ExperienceItem key={exp.id} exp={exp} />))}</div>
                <Footer />
              </A4Page>
            ))}
          </div>
