    ...
    <div className="mb-6 relative">
      <div className="flex justify-between items-end mb-1">
        ...
        <div className="flex items-center gap-1 bg-slate-100 rounded-t-lg px-2 py-1 border border-slate-200 border-b-0 absolute right-0 top-0 transform -translate-y-full">
          ...
          {llmTools.map((tool) => (
            <button key={tool.name} onClick={() => copyAndOpenAI(tool.url)} className="p-1 hover:bg-white rounded transition-all hover:scale-110 grayscale hover:grayscale-0 opacity-70 hover:opacity-100" title={`Copier texte et ouvrir ${tool.name}`}>
...
// --- COMPOSANT D'IMPRESSION (Page Unique) ---
const PrintPage = ({ children }) => (
  <div className="cv-page relative overflow-hidden flex flex-col shadow-2xl bg-white mb-10 mx-auto">
    {children}
  </div>
);
...
export default function App() {
  ...
  const [isPrinting, setIsPrinting] = useState(false); // État pour le mode impression
  ...
  // --- GESTION IMPRESSION NATIVE ---
  const handlePrint = () => {
    setIsPrinting(true);
    // On attend un cycle de rendu pour que le DOM se mette à jour en mode "isPrinting"
    setTimeout(() => {
      window.print();
      setIsPrinting(false);
      handleEmail(); // Proposer le mail après l'impression
    }, 500);
  };
  ...
  // --- CONTENU DES PAGES (Séparé pour réutilisation en mode print) ---
  const PagesContent = () => (
    <>
      {/* PAGE 1 */}
      <PrintPage>
  ...
  // --- MODE IMPRESSION EXCLUSIF (Rendu Conditionnel) ---
  if (isPrinting) {
    return (
      <div className="print-only-mode">
        <PagesContent />
        <style>{`
          @media print {
            body { margin: 0; padding: 0; background: white; }
            @page { size: A4; margin: 0; }
            .print-only-mode { width: 210mm; }
            /* Les styles globaux de index.css gèrent le reste */
          }
        `}</style>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row h-screen overflow-hidden font-sans">
      <div className="w-full md:w-[550px] bg-white border-r border-slate-200 flex flex-col h-full z-10 shadow-xl print:hidden">
        ...
        <div className="p-6 border-b border-slate-100 bg-white sticky top-0 z-20">
          ...
          <div className="flex gap-2">
            ...
            {step < 4 ? <Button onClick={() => setStep(s => Math.min(4, s + 1))} className="flex-[2]">Suivant <ArrowRight size={16} /></Button> : <Button onClick={handlePrint} className="flex-[2] bg-slate-900 hover:bg-black"><Download size={16}/> Exporter PDF</Button>}
          </div>
        </div>
        
        {/* Wizard Content (Step 1-4 Logic) */}
        <div className="flex-1 overflow-y-auto px-6 py-8 custom-scrollbar">
           {/* ... (Je ne répète pas tout le code du wizard ici pour la clarté, il est identique à la version précédente sauf pour les modifications de texte déjà faites) */}
           {/* Intégrez ici les blocs switch case step 1 à 4 comme précédemment */}
           {step === 1 && (
            <div className="space-y-6 animate-in slide-in-from-right duration-300">
              ...
              <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg flex gap-3"><div className="text-[#2E86C1] shrink-0 mt-0.5"><HelpCircle size={18} /></div><div><h4 className="text-xs font-bold text-[#2E86C1] uppercase mb-1">Comment utiliser l'IA ?</h4><p className="text-[11px] text-slate-600 leading-tight">Cliquez sur les logos (ChatGPT, etc.) au-dessus des zones de texte pour copier votre contenu avec une consigne d'amélioration automatique.</p></div></div>
              ...
              <RichTextarea label="Bio / Résumé" value={cvData.profile.summary} onChange={(val) => handleProfileChange('summary', val)} maxLength={400} placeholder="Décrivez votre profil..." />
              <div className="bg-white p-4 rounded-xl border border-slate-200"><label className="text-xs font-bold text-[#333333] uppercase block mb-3">Bandeau Technos</label><LogoSelector onSelect={addTechLogo} label="Ajouter une techno" /><div className="flex flex-wrap gap-2 mt-4">{cvData.profile.tech_logos.map((logo, i) => (<div key={i} className="relative group bg-slate-100 p-2 rounded-md border border-slate-200"><img src={logo.src} className="w-6 h-6 object-contain" alt={logo.name} /><button onClick={() => removeTechLogo(i)} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition-opacity"><X size={10} /></button></div>))}</div></div>
            </div>
           )}
           {step === 2 && (
            <div className="space-y-6 animate-in slide-in-from-right duration-300">
               ...
               <div className="mt-8 border-t border-slate-100 pt-6">
                 ...
                 {Object.entries(cvData.skills_categories).map(([cat, skills]) => (
                   <div key={cat} className="mb-6 p-4 bg-white border border-slate-200 rounded-xl">
                      ...
                      <div className="flex items-center gap-2 bg-blue-50/50 p-2 rounded-lg"><input className="flex-1 px-2 py-1 text-xs border border-slate-200 rounded" placeholder="Compétence (ex: Jira)" value={newSkillsInput[cat]?.name || ''} onChange={(e) => updateNewSkillInput(cat, 'name', e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addSkillToCategory(cat)} /><div className="flex items-center gap-1"><span className="text-[9px] text-slate-400 font-bold">NOTE:</span><HexagonRating score={newSkillsInput[cat]?.rating || 3} onChange={(r) => updateNewSkillInput(cat, 'rating', r)} /></div><Button variant="primary" className="px-2 py-1 text-xs h-auto" onClick={() => addSkillToCategory(cat)}><Plus size={12}/></Button></div>
                   </div>
                 ))}
               </div>
             </div>
            ...
            <div className="space-y-8 animate-in slide-in-from-right duration-300">
              ...
              {cvData.experiences.map((exp) => (
                <div key={exp.id} className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm relative group mb-4">
                  ...
                  <div className="mb-4"><span className="text-xs font-bold text-[#333333] uppercase block mb-2">Logo Client</span><div className="flex items-center gap-4"><div className="w-12 h-12 bg-slate-50 border border-slate-200 rounded-full flex items-center justify-center overflow-hidden shrink-0">{exp.client_logo ? <img src={exp.client_logo} className="w-full h-full object-cover" /> : <ImageIcon size={20} className="text-slate-300"/>}</div><div className="flex-1"><LogoSelector label="" onSelect={(logo) => updateExperience(exp.id, 'client_logo', logo.src)} /></div></div></div>
      ...
      <style>{`
        ...
        @media print {
          ...
          .flex-1.bg-slate-800 { display: block !important; overflow: visible !important; height: auto !important; background: white !important; }
          .print-container { transform: none !important; margin: 0 !important; gap: 0 !important; width: 100% !important; display: block !important; }
          .cv-page { width: 100% !important; height: auto !important; margin: 0 !important; page-break-after: always; box-shadow: none !important; }
        }
      `}</style>
    </div>
  );
