...
export default function App() {
  ...
  // --- LOGIQUE DE PAGINATION MANUELLE (A4 = 297mm) ---
  // On découpe les expériences en pages. On considère qu'on peut mettre environ 2 grosses expériences par page.
  // Si une page a moins de 2 expériences (ex: la dernière), on pourra la centrer verticalement.
  const ITEMS_PER_PAGE = 2;
  const experiencePages = [];
  for (let i = 0; i < cvData.experiences.length; i += ITEMS_PER_PAGE) {
    experiencePages.push(cvData.experiences.slice(i, i + ITEMS_PER_PAGE));
  }
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row h-screen overflow-hidden font-sans">
      ...
      {/* --- WIZARD UI (Code inchangé pour la saisie) --- */}
      <div className="w-full md:w-[550px] bg-white border-r border-slate-200 flex flex-col h-full z-10 shadow-xl print:hidden">
         {/* ... (Je reprends le header et le contenu du wizard de la version précédente qui était fonctionnelle) ... */}
         {/* Pour gagner de la place ici, imaginez le code du Wizard identique : Profil, Soft Skills, Formation, Expériences */}
         ...
         {/* WIZARD CONTENT - COPIÉ DE LA VERSION PRECEDENTE (Fonctionnel) */}
         <div className="flex-1 overflow-y-auto px-6 py-8 custom-scrollbar">
            ...
            {step === 3 && (
              <div className="space-y-8 animate-in slide-in-from-right">
                ...
                {cvData.education.map((edu, i) => (<div key={i} className="bg-slate-50 p-4 rounded-lg border border-slate-200 mb-3 relative group"><button onClick={() => removeEducation(i)} className="absolute top-2 right-2 text-slate-300 hover:text-red-500"><Trash2 size={14}/></button><Input label="Diplôme" value={edu.degree} onChange={(v) => updateEducation(i, 'degree', v)} /><div className="grid grid-cols-2 gap-2"><Input label="Année" value={edu.year} onChange={(v) => updateEducation(i, 'year', v)} /><Input label="Lieu" value={edu.location} onChange={(v) => updateEducation(i, 'location', v)} /></div></div>))}
      ...
      {/* --- ZONE D'APERÇU & D'IMPRESSION (SÉPARÉE EN PAGES A4) --- */}
      <div className="flex-1 bg-slate-800 overflow-hidden relative flex flex-col items-center">
        ...
        <div className="flex-1 overflow-auto w-full p-8 flex justify-center custom-scrollbar">
          <div style={{ transform: `scale(${zoom})`, transformOrigin: 'top center', marginBottom: '100px' }}>
            ...
            {/* PAGE 2 : FORMATION & COMPÉTENCES (FIXE) */}
            <A4Page>
               <CornerTriangle customLogo={cvData.smileLogo} />
            ...
            {/* PAGES 3+ : EXPÉRIENCES (DYNAMIQUES) */}
            {experienceChunks.map((chunk, pageIndex) => (
              <A4Page key={pageIndex}>
                ...
                {/* Si c'est la dernière page et qu'elle est peu remplie, on centre le contenu pour faire joli */}
