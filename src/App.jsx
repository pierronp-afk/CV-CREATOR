...
export default function App() {
  ...
  const uploadJSON = (e) => { 
    ...
    reader.onload = (ev) => { try { setCvData(JSON.parse(ev.target.result)); } catch (err) { alert("Invalide"); } }; 
  ...
  const handlePDFImport = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({ contents: [{ parts: [{ text: `Texte du CV : ${rawText}` }] }], systemInstruction: { parts: [{ text: systemPrompt }] }, generationConfig: { responseMimeType: "application/json" } })
      });
    ...
    } catch (err) { console.error(err); alert("Erreur import."); } finally { setIsImporting(false); }
  };
            ...
            <div className="space-y-6 animate-in slide-in-from-right transition-all">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                ...
                <div className="p-3 border border-slate-200 bg-slate-50 rounded-lg flex flex-col gap-2">
                  <span className="text-[10px] font-bold text-slate-600 uppercase flex items-center justify-between">Photo {cvData.isAnonymous && <span className="text-[9px] bg-red-100 text-red-600 px-1 rounded uppercase">Cachée</span>}</span>
          ...
          <div className="print-container flex flex-col origin-top transition-transform duration-300 gap-10" style={{ transform: `scale(${zoom})`, marginBottom: `${zoom * 100}px` }}>
            ...
            <A4Page>
              ...
              <div className="pt-36 px-16 pb-0 flex-shrink-0 text-left">
                 ...
                 <div className="text-lg text-[#666666] font-medium uppercase tracking-widest mb-4 border-l-4 border-[#2E86C1] pl-4 text-left">{cvData.profile.main_tech}</div>
              </div>
                  ...
                  <div className="flex justify-center gap-12 relative z-10 px-10 flex-shrink-0 mt-12">
                    {cvData.soft_skills.map((skill, i) => (
                      <div key={i} className="relative w-40 h-44 flex items-center justify-center">
                        ...
                        <span className="relative z-10 text-white font-bold text-sm uppercase text-center px-4 leading-tight font-montserrat">{String(skill || "Soft Skill")}</span>
                      </div>
                    ))}
                  </div>
              </div>
